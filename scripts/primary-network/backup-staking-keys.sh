...
if [ -z "$HOSTNAME" ]; then
    ...
    ansible-playbook -i "$INVENTORY" playbooks/primary-network/backup-staking-keys.yml
else
    echo "Backing up staking keys for $HOSTNAME..."
    cd "$REPO_ROOT/ansible"
    ansible-playbook -i "$INVENTORY" playbooks/primary-network/backup-staking-keys.yml --limit "$HOSTNAME"
fi

echo ""
echo "Backup complete! List backups with:"
echo "  aws s3 ls s3://\$(terraform -chdir=../terraform/primary-network/$CLOUD output -raw staking_keys_bucket)/"
