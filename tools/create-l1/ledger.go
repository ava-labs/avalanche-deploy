...
// It implements the secp256k1fx.Keychain interface for use with avalanchego wallets
...
// Addresses implements secp256k1fx.Keychain interface
func (kc *LedgerKeychain) Addresses() set.Set[ids.ShortID] {
	return kc.addresses
}

// Get implements secp256k1fx.Keychain interface
// Note: This returns nil for private key since Ledger never exposes it
// The signing is handled through SignHash method instead
func (kc *LedgerKeychain) Get(addr ids.ShortID) (*secp256k1.PrivateKey, bool) {
	// Ledger never exposes private keys - signing must go through SignHash
	return nil, false
}
...
// findLedgerWithRetry attempts to find the Ledger device with retries
...
// getPublicKeyWithRetry gets public key from Ledger with retries
func getPublicKeyWithRetry(device *ledger.LedgerAvalanche, path string) (*ledger.ResponseAddr, error) {
	...
	delay := ledgerRetryDelay
	for i := 0; i < ledgerMaxRetries; i++ {
		// GetPubKey(path, show, hrp, chainid)
		// show=false: don't require user confirmation for just getting the address
		// hrp="avax": human-readable prefix for Avalanche addresses
		// chainid="P": P-Chain for platform operations
...
// signHashWithRetry signs a hash using Ledger with retries
func signHashWithRetry(device *ledger.LedgerAvalanche, path string, hash []byte) ([]byte, error) {
	...
	for i := 0; i < ledgerMaxRetries; i++ {
		...
		if signErr == nil && response != nil {
			// ResponseSign.Signature is map[string][]byte where key is the signing path
			if sig, ok := response.Signature[path]; ok {
				return sig, nil
			}
			// Fallback: return first signature if path key doesn't match exactly
