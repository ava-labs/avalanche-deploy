...
## What this is
Defines the JSON messages exchanged over vsock (one JSON value per connection — no length prefix) between the host process and the BLS-signing enclave VM. It is a shared leaf package: `api/awsnitro` imports it as the host (client) side and `enclave/` imports it as the enclave (server) side. It contains only types and constants — no I/O logic.
...
- **`VSockPort` (5000)** — per-operation `Request` → `Response`. `Request.Type` is one of `RequestSign` (warp/ICM, signs with `DSTSign`), `RequestSignPoP` (signs with `DSTPoP`), or `RequestPublicKey`. `Response` returns `Result` (96-byte signature or 48-byte key; base64 on the wire, as JSON encodes `[]byte`) or a non-empty `Error`.

`MaxMessageSize` caps reads on both sides at 1 MB (enforced with `io.LimitReader`). Credentials cross the boundary, but the **plaintext BLS key never leaves the enclave** — that is the security win over the plain cloud-KMS backends.
...
- Truncated/oversized message errors → the message exceeds `MaxMessageSize` (the JSON decode fails with an unexpected-EOF once the read cap truncates it); both sides must use these constants.
