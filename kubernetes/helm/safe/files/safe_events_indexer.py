# Patched copy of safe_transaction_service/history/indexers/safe_events_indexer.py
# from safe-transaction-service v5.42.1 (the version pinned in values.yaml).
# Single functional change: skip the simulated sender-side ether transfer when the
# recipient is an indexed Safe (upstream issue #1556 - duplicated Safe->Safe
# transfers on event-indexed/L2 networks). Mounted over the original via the
# txs-patches ConfigMap. MUST be regenerated when the TXS image tag changes.
# NOTE: the ansible role ships its own copy generated from v5.40.1 — the two
# files are version-specific and intentionally NOT identical.
import datetime
    ...
    def _process_decoded_element(
        ...
        if event_name == "ProxyCreation" or event_name == "SafeSetup":
            ...
            if args["value"] and not data:  # Simulate ether transfer
                # avalanche-deploy patch for safe-global/safe-transaction-service#1556:
                # when the recipient is a Safe indexed by this service, its
                # SafeReceived event already produces the canonical transfer row,
                # so simulating the sender side too duplicates every Safe->Safe
                # native send in both Safes' history. Only simulate when no
                # SafeReceived event will be processed (EOA or external recipient).
                recipient = args.get("to")
                if not (
                    recipient
                    and SafeContract.objects.filter(address=recipient).exists()
                ):
